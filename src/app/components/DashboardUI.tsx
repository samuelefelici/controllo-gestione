...
/* ═══════ Main Component ═══════ */
export default function DashboardUI({ data, loading, error, period, setPeriod, setData, isPublic = false, clientId = "" }: DashboardUIProps) {
  ...
  const pnl = useMemo(() => {
    ...
    const altreEntrateBanca = incomeBD
      .filter((c: any) => { const n = (c.name || "").toUpperCase(); return n !== "INCASSO"; })
  ...
  if (error || !data || data.error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 gap-4">
        ...
        <p className="text-xl text-slate-400">{error || "Nessun dato disponibile"}</p>
        <p className="text-sm text-slate-600">
          {isPublic ? "Verifica che il link sia corretto o contatta il tuo consulente" : "Carica prima i file dal pannello di gestione"}
        </p>
        {!isPublic && clientId && (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-3">
              {isPublic ? (
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-sky-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs shadow-lg shadow-sky-500/20">FF</div>
              ) : (
          ...
          return (
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                {[
                  { label: "Margine Operativo", value: `${pnl.kpi.margineLordoPct.toFixed(1)}%`, sub: fmt(pnl.totaleRicavi - costiOpTot), color: signal(pnl.kpi.margineLordoPct, 35, 20), dot: signalBg(pnl.kpi.margineLordoPct, 35, 20) },
                  { label: "Risultato Netto %", value: `${pnl.kpi.ebitdaPct.toFixed(1)}%`, sub: fmt(pnl.risultatoNetto), color: signal(pnl.kpi.ebitdaPct, 10, 3), dot: signalBg(pnl.kpi.ebitdaPct, 10, 3) },
                  ...
                  { label: "Liquidità", value: fmt(ba.closing_balance || 0), sub: `${pnl.kpi.daysOfCash.toFixed(0)} giorni di cassa`, color: signal(pnl.kpi.daysOfCash, 60, 30), dot: signalBg(pnl.kpi.daysOfCash, 60, 30) },
                  { label: "Fatt. / Dipendente", value: fmt(pnl.kpi.fatturatoPerDip), sub: `su ${pa.employee_count || 0} dip.`, color: "text-sky-400", dot: "bg-sky-500" },
                ].map((kpi, i) => (
                  ...
                  <Card className="p-5">
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <AreaChart data={data.bank.daily_balance}>
                        <defs>
                          <linearGradient id="balGradOv" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <Area type="monotone" dataKey="balance" name="Saldo" stroke="#10b981" fill="url(#balGradOv)" strokeWidth={2} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </Card>
                )}
                    ...
                    <BarChart data={salesData} layout="vertical" margin={{ left: 100 }} onClick={(e: any) => {
                      ...
                      <Bar dataKey="net_sales" name="Netto" radius={[0, 4, 4, 0]} cursor="pointer">
                        {salesData.map((entry: any, idx: number) => (
                          <Cell key={idx} fill={activeFilter?.type === "sales_cat" && activeFilter.value !== entry.category_name ? "#1e293b" : "#0ea5e9"} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                      ...
                      <tr key={i}
                        ...
                        className={`border-b border-slate-800/30 transition cursor-pointer ${
                          activeFilter?.type === "sales_cat" && activeFilter.value === c.category_name
                            ...
                            : activeFilter?.type === "sales_cat" ? "opacity-40 hover:opacity-70" : "hover:bg-slate-800/20"
                        }`}>
        ...
        {tab === "costs" && (
          <>
                      ...
                      <div key={i}
                        ...
                        className={`flex items-center gap-3 cursor-pointer rounded-lg px-2 py-1 -mx-2 transition ${
                          activeFilter?.type === "cost_cat" && activeFilter.value === item.name
                            ...
                            : activeFilter?.type === "cost_cat" ? "opacity-40 hover:opacity-70" : "hover:bg-slate-800/40"
                        }`}>
                      ...
                      <div key={i}
                        ...
                        className={`flex items-center gap-3 cursor-pointer rounded-lg px-2 py-1 -mx-2 transition ${
                          activeFilter?.type === "supplier" && activeFilter.value === item.name
                            ? "bg-sky-950/50 ring-1 ring-sky-500/30"
                            : activeFilter?.type === "supplier" ? "opacity-40 hover:opacity-70" : "hover:bg-slate-800/40"
                        }`}>
                        <div className="w-2 h-2 rounded-full" style={{ background: COLORS[i % COLORS.length] }} />
                        <span className="text-xs text-slate-400 flex-1 truncate">{item.name}</span>
            ...
            {activeFilter?.type === "cost_cat" && (() => {
              ...
              const allTx = [
                ...filteredBankTx.map((tx: any) => ({ date: tx.transaction_date, description: tx.counterpart || tx.description?.substring(0, 80) || "—", source: "Banca", amount: Math.abs(tx.amount) })),
                ...filteredAmexTx.map((tx: any) => ({ date: tx.operation_date, description: tx.description || "—", source: "Amex", amount: Math.abs(tx.amount_eur) })),
              ...
              return allTx.length > 0 ? (
                <Card className="overflow-hidden">
                  <div className="px-5 py-3 border-b border-slate-800/60 flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-white">Movimenti — <span className="text-sky-400">{activeFilter.value}</span></h3>
                      ...
                      <tbody>
                        {allTx.map((tx, i) => (
                          <tr key={i} className="border-b border-slate-800/20 hover:bg-slate-800/20 transition">
                            ...
                            <td className="p-3"><span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${tx.source === "Banca" ? "bg-blue-950 text-blue-400" : "bg-purple-950 text-purple-400"}`}>{tx.source}</span></td>
          ...
          <>
            ...
            {(() => {
              ...
              for (const item of ibData) {
                if (item.value > 0) { steps.push({ name: item.name, base: r, up: item.value, down: 0, total: r + item.value }); r += item.value; }
              }
              for (const item of cbData) {
                if (item.value > 0) { steps.push({ name: item.name, base: r - item.value, up: 0, down: item.value, total: r - item.value }); r -= item.value; }
              }
              if (totalUsciteAmex > 0) {
                steps.push({ name: "Amex", base: r - totalUsciteAmex, up: 0, down: totalUsciteAmex, total: r - totalUsciteAmex }); r -= totalUsciteAmex;
              }
                  ...
                  <tbody>
                    {(data.bank?.transactions || [])
                      ...
                      <tr key={i} className={`border-b border-slate-800/20 hover:bg-slate-800/20 transition ${tx.amount > 0 ? "bg-emerald-950/10" : "bg-red-950/10"}`}>
                        ...
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${tx.amount > 0 ? "bg-emerald-950 text-emerald-400" : "bg-red-950 text-red-400"}`}>
                            {tx.subcategory || tx.category}
                          </span>
                        </td>
                        <td className="p-3">
                          {!isPublic && tx.amount > 0 ? (
                            <select
                            ...
                            <span className="text-[10px] text-slate-400">{tx.income_category || "Auto"}</span>
                          ) : (
                  ...
                  return (
                    <div key={si}>
                      ...
                      <div className={`flex justify-between px-5 py-3 ${c.bg} font-bold text-sm border-b border-slate-800/60`}>
                        ...
                        <span className={`font-mono ${c.text}`}>{isRevenue ? "" : "-"}{fmt2(sec.subtotal)}</span>
                      </div>
